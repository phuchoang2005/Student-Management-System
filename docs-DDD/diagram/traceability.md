...
| 2 | `getStudents` | `GET /students` | UC3 | **`list-students.mmd`** | `StudentApplicationService.listStudents` | — (paged read) | 200 `PageStudentResponse` | — |
| 3 | `getStudentById` | `GET /students/{studentId}` | UC2 | — | `StudentApplicationService.getStudent` | — | 200 `StudentResponse` | 404 |
| 4 | `updateStudent` | `PUT /students/{studentId}` | UC4 | **`update-student.mmd`** | `StudentApplicationService.updateStudent` | uniqueness checks must **exclude self** | 200 `StudentResponse` | 400, 404, 409 |
...
| 7 | `getBooks` | `GET /books` | UC17 | *(pattern of `list-students.mmd`)* | `BookApplicationService.listBooks` | — (paged read) | 200 `PageBookResponse` | — |
| 8 | `getBookById` | `GET /books/{bookId}` | UC16 | — | `BookApplicationService.getBook` | — | 200 `BookResponse` | 404 |
| 9 | `updateBook` | `PUT /books/{bookId}` | UC18 | *(pattern of `update-student.mmd`)* | `BookApplicationService.updateBook` | unique `isbn` excluding self; **must not change `ownerId`** | 200 `BookResponse` | 400, 404, 409 |
...
| 12 | `getCourses` | `GET /courses` | UC8 | *(pattern of `list-students.mmd`)* | `CourseApplicationService.listCourses` | — (paged read) | 200 `PageCourseResponse` | — |
| 13 | `getCourseById` | `GET /courses/{courseId}` | UC7 | — | `CourseApplicationService.getCourse` | — | 200 `CourseResponse` | 404 |
| 14 | `updateCourse` | `PUT /courses/{courseId}` | UC9 | *(pattern of `update-student.mmd`)* | `CourseApplicationService.updateCourse` | unique `courseCode` excluding self; `credits > 0` | 200 `CourseResponse` | 400, 404, 409 |
...
## Views that are not per-operation

Four diagrams cut across the table above instead of realising a row of it. They exist because the same rule shows up in many operations, and repeating it in every sequence diagram is how the rule drifts.

| View | File | Answers |
|---|---|---|
| Error mapping | `architecture/error-mapping.mmd` | which exception becomes which status and code — the diagram form of the error table above |
| Book ownership lifecycle | `states/book-ownership.mmd` | every state a book's ownership can be in, and all four ways it changes (ops 5, 16, 17, 19) |
| Enrollment lifecycle | `states/enrollment.mmd` | the two enrollment states, and all four operations that destroy one (ops 5, 15, 20, 21) |
| Deployment | `deployment/deployment.mmd` | what actually runs, and which of the gaps below block it |

The state diagrams are also the transition checklist for the state-based tests in `../plan-springBootTestPlan.prompt.md`: every arrow, and every rejected transition in the notes, is one test case.
