...
class RbacMatrixIntegrationTest {
  ...
  private static Stream<Arguments> enrollmentWrites(String role) {
    return Stream.of(
        ...
        Arguments.of(HttpMethod.DELETE, "/api/v1/enrollments/1/CS101", role));
  }
  ...
  // TC-XC-006. Detail-endpoint 200 coverage per role already exists in StudentLookupIntegrationTest
  // / BookLookupIntegrationTest / EnrollmentLookupIntegrationTest -- this only completes the
  // search-endpoint slice of the matrix. No "list all enrollments" endpoint exists in this API
  // (EnrollmentController only has detail/create/end), so enrollments aren't part of this slice.
  @ParameterizedTest(name = "{0} can read {1}")
  @MethodSource("everyDomainRoleCanSearch")
  void everyDomainRoleCanReachSearchEndpoints(String role, String path) throws Exception {
    mockMvc.perform(get(path).with(user("caller").roles(role))).andExpect(status().isOk());
  }

  static Stream<Arguments> everyDomainRoleCanSearch() {
    Stream.Builder<Arguments> args = Stream.builder();
    for (String role : new String[] {"REGISTRAR", "LIBRARIAN", "COURSE_ADMINISTRATOR", "STUDENT"}) {
      for (String path : new String[] {"/api/v1/students", "/api/v1/books", "/api/v1/courses"}) {
