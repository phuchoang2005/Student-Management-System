 ...
 * before this ticket. Every STUDENT session here is a real registered+logged-in account (mirroring
...
class OwnRecordsScopingIntegrationTest {
  ...
  private record Student(long id, String code, MockHttpSession session) {}
  ...
  private void addBookOwnedBy(String isbn, long ownerId) throws Exception {
    mockMvc
        .perform(
            post("/api/v1/books")
                .with(user("librarian").roles("LIBRARIAN"))
  ...
  void aStudentReadingTheirOwnBookDetailSucceeds() throws Exception {
    ...
    addBookOwnedBy("ISBN-1006", student.id());
  ...
  void aStudentReadingAnotherStudentsOwnedBookDetailIsForbidden() throws Exception {
    ...
    addBookOwnedBy("ISBN-1008", other.id());
  ...
  @Test
  void aStudentsBookSearchIsScopedToTheirOwnBooksRegardlessOfTheOwnerQueryParam() throws Exception {
    Student student = aStudent("S01010", "scope.1010@example.edu");
    Student other = aStudent("S01011", "scope.1011@example.edu");
    addBookOwnedBy("ISBN-1010", student.id());
    addBookOwnedBy("ISBN-1011", other.id());
    ...
    // Passing another student's id as owner is silently overridden, never rejected or honored.
    mockMvc
        .perform(get("/api/v1/books").session(student.session()).param("owner", String.valueOf(other.id())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.content.length()").value(1))
        .andExpect(jsonPath("$.content[0].isbn").value("ISBN-1010"));
  }
  ...
  @Test
  void aStudentReadingTheirOwnEnrollmentDetailSucceeds() throws Exception {
    Student student = aStudent("S01012", "scope.1012@example.edu");
    createCourse("CS1012");
    enroll(student.id(), "CS1012");

    mockMvc
        .perform(get("/api/v1/enrollments/" + student.id() + "/CS1012").session(student.session()))
        .andExpect(status().isOk());
  }

  @Test
  void aStudentReadingAnotherStudentsEnrollmentIsForbiddenWhetherOrNotItExists() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/enrollments/" + other.id() + "/CS1013A").session(student.session()))
        .andExpect(status().isForbidden());
    // ...or doesn't -- no existence signal leaks through a differently-timed 404.
    mockMvc
        .perform(get("/api/v1/enrollments/" + other.id() + "/CS1013B").session(student.session()))
        .andExpect(status().isForbidden());
  }
  ...
  void staffRolesAreNeverScopedRegardlessOfOwnership() throws Exception {
    ...
    addBookOwnedBy("ISBN-1015", student.id());
