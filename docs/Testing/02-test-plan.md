...
| [use-cases.md](../BA-docs/use-cases.md) | 23 use cases (UC-1–UC-23), actors, main/alternate/exception flows |
| [user-stories.md](../BA-docs/user-stories.md) | 18 user stories (US-1.1–US-6.3), acceptance criteria |
...
| 6 | `identity` (login / change password / view initial password, beyond auto-provisioning) | Needs a provisioned account (from step 2) to log in with | `student` |
...
| `identity` | UC-1 (tail), UC-21, UC-22, UC-23 | Staff account creation flow — explicitly out of scope, assumed pre-seeded (`04-authentication-authorization.md` §2.2); forgot-password/MFA/SSO — out of scope (`04-authentication-authorization.md` §7) |
...
| 4 | Staff (Registrar/Librarian/Course Administrator) accounts are assumed pre-seeded, per `04-authentication-authorization.md` §2.2, with no defined provisioning flow | Test environments need a documented, out-of-band way to seed these 3 accounts, or role-based test cases can't run | Addressed in [04-test-data-preparation.md](./04-test-data-preparation.md) §1 with a defined seed set |
