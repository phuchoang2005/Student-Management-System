...
Software Architecture — v0.1 addendum. One section per `SA-docs/` document that `Benchmark/07-improvement-roadmap.md` and `08-hazard-fix-specs.md`'s `IP-01`…`IP-11` fixes touch, change, or contradict. Nothing here is a code change or a redrawn diagram — every item below is either a documented fact about the system as it exists today, a forward note about an edit due once a fix lands, or an open decision presented for someone to make. **Written before any fix but `IP-09` had landed** — Sprints 9 and 10 (`PM-docs/epic-k-sprint-plan.md`) have since shipped `IP-01`–`IP-08`/`IP-11` (only `IP-10` remains unscheduled), so several sections below now describe a "not yet landed"/"still open" state that no longer holds. §2 and §7.2 have been updated in place to reflect what shipped (the `IP-05` pagination decision); §1, §4, §5, and §6 have not been re-verified against the post-Sprint-9/10 codebase and should be read as-written, not as current status — cross-check `epic-k-sprint-plan.md` before relying on any "once `IP-*` lands" phrasing below.
...
- **Observability is real, permanent, and undocumented.** §5 says nothing about actuator, Micrometer, or metrics anywhere. As of this document, that's inaccurate by omission: `management/pom.xml` carries `spring-boot-starter-actuator` and `micrometer-registry-prometheus` as permanent dependencies (not benchmark-session scaffolding); `application-benchmark.properties` exposes `/actuator/health`, `/actuator/metrics`, and `/actuator/prometheus` only under the `benchmark` Spring profile, on a dedicated `:8081` management port; `SecurityConfig.java:161-170` gates `/actuator/**` on the main port behind `SYSTEM_ADMINISTRATOR` and permits only `/actuator/health` unauthenticated. On top of that, `Benchmark/benchmark-strategy/06-dashboard-building.md` specifies a full six-dashboard Grafana/Prometheus stack (Benchmark Overview, HTTP & Load Testing, JVM Runtime, Spring Boot Runtime, MySQL Performance, Performance Correlation) built against exactly this metrics surface. **This is `06-conclusions-and-recommendations.md` §6 recommendation #9 (`IP-09`) already done, and exceeded** — `08-hazard-fix-specs.md`'s `IP-09` entry is being corrected to reflect this (see that document). §5 should gain an observability row describing this capability; it currently reads as if the system has none.

  Worth flagging alongside this: `06-dashboard-building.md` is a sixth document in a folder (`benchmark-strategy/`) that its own `README.md`, `01-benchmark-strategy.md` §11's out-of-scope list, and the top-level `Benchmark/README.md` all used to describe as a "five-part design" — leaving the observability design out of every route a reader would take to find it. **That gap is now closed**: all three were corrected when the v0.0 and v0.1 document sets were merged into `docs/`, and `06-dashboard-building.md` now appears in the folder's own reading order. It was a Benchmark cross-reference gap rather than an SA-docs one, and is recorded here because it is what surfaced this section's own gap — which remains open: §5 still has no observability row.

## 2. `api-specification.md`

Two open decisions this document doesn't make, both already load-bearing on the API contract:

- **Decision #8 (pagination defaults/cap) is superseded by `IP-05`, for most but not all of §3's endpoints.** §3 documents the current envelope verbatim: `{content: [...], page, size, totalElements, totalPages}`, and decision #8 fixes the default/cap/invalid-page behavior around it. `IP-05`'s page-number-vs-cursor decision has been made: Prev/Next-only, keyset (cursor) pagination, shipped for `GET /students`, `/books`, `/courses`, `/enrollments`, `/me/books`, `/me/courses` (`shared/paging/CursorPage.java`, returning `{content: [...], nextCursor}` — no `totalElements`/`totalPages`, since no use case ever promised a page count, `BA-docs/non-functional-requirements.md` §3.1). Staff-account listing (`GET /staff-accounts`) was **not** converted and still returns the original `PageResponse` envelope decision #8 describes. The contract is now a documented split, not one decision superseded cleanly by another — `api-specification.md` §3 needs a successor decision recorded here: decision #8 stays as-is for staff accounts, and a new decision documents the cursor envelope for the six converted endpoints.
- **§6's "no rate limiting" is an explicit, named decision — and `IP-08` has one option that reverses it and one that doesn't.** `IP-08` proposes either a bulkhead (a dedicated, bounded thread pool for the auth endpoint) or rate-limiting/`429` once a login queue passes a threshold. A bulkhead is invisible at the API layer — no client-observable change, no contract edit, §6 stays correct as written. Rate-limiting is not: it introduces a new response the API can return that §6 currently rules out by name. If that path is chosen, it is the same shape of move `01-benchmark-strategy.md` §2.1 already made once, reversing `Testing/01-test-strategy.md`'s load-testing exclusion "deliberately, and with a reason" — §6 would need the same treatment: the decision restated with the reason for reversing it, not silently dropped. This document does not choose between the two options; see `BA-docs/non-functional-requirements.md` §3.2 for the same open choice at the business-requirements layer.
...
**Still not drawn here as an "after" diagram, but no longer because the decision is open — it's been made and shipped.** `IP-05`'s page-number-vs-cursor decision resolved to Prev/Next-only, cursor-based pagination (`BA-docs/non-functional-requirements.md` §3.1; `Benchmark/10-customer-performance-summary.md` §4), landed for the student/book/course search endpoints and `/me/books`/`/me/courses` behind `shared/paging/CursorPage.java`. `UC-13`/`UC-14`/`UC-15`/`UC-16`'s four diagrams should now be redrawn — `Repo->>DB: SELECT ... LIMIT/OFFSET (+ COUNT for totalElements)` becomes `WHERE <key> > :cursor ORDER BY <key> LIMIT :limit` (no count query at all, per `IP-03`'s coupling note resolving the same way `IP-05` did), and the `200 OK` response shape becomes `{content, nextCursor}`. Not redrawn in this pass — this document's own scope is impact-flagging, not redrawing v0.0 diagrams — but the open condition that justified deferring it no longer holds, so this is now a ready-to-do edit rather than a blocked one.
