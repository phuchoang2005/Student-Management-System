...
Benchmark Documentation — v0.1 addendum, Part 3. Compares [`2026-08-29-S2-156d247.md`](./result/2026-08-29-S2-156d247.md) (the first v0.1-era run, against the current code — Sprint 9 + Sprint 10, `IP-01`…`IP-09`/`IP-11` landed, `IP-10` deferred) to [`Benchmark/06-conclusions-and-recommendations.md`](./06-conclusions-and-recommendations.md) and the run records it's built from. Regression verdicts below follow [`05-baseline-and-reporting.md`](./benchmark-strategy/05-baseline-and-reporting.md) §2's band table (Improvement / No change / Investigate / Block), applied per scenario.
...
Per this folder's fast-smoke scope, these need a follow-up run before they can be called verified. `Benchmark/07-improvement-roadmap.md`'s phase exit criteria state what each needs:
...
3. **Schedule the deferred scope (§6)** as the next v0.1-era run in [`result/`](./result/) once §3's fix lands, so the follow-up run's before-numbers aren't still measuring the regression.
