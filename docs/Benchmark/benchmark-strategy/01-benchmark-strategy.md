...
- The dashboards and metrics surface the numbers are read through — see [06-dashboard-building.md](./06-dashboard-building.md).
- Actual benchmark code — deliberately not produced by this documentation task, on the same basis as `Testing/` and `PM-docs/`: these six documents are the design that later drives a real harness.
