...
This is documentation only — **no benchmark code is included here.** The six documents below specify a harness (k6 scenarios under a `bench/` directory, a deterministic dataset generator, `make bench` targets) — and the observability stack that makes its numbers attributable — precisely enough to build, on the same basis that [`Testing/`](../../Testing/) specified a test suite before one existed and [`PM-docs/`](../../PM-docs/) specified an implementation plan before there was an implementation.
...
6. **[06-dashboard-building.md](./06-dashboard-building.md)** — what the numbers are read through: six purpose-driven Grafana dashboards (Benchmark Overview, HTTP & Load Testing, JVM Runtime, Spring Boot Runtime, MySQL Performance, Performance Correlation) built on the actuator/Micrometer surface H8 calls for, so a latency figure can be attributed rather than only observed.
