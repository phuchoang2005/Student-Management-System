...
**Still not drawn here as an "after" diagram, but no longer because the decision is open — it's been made and shipped.** `IP-05`'s page-number-vs-cursor decision resolved to Prev/Next-only, cursor-based pagination (`docs-v01/BA-docs/non-functional-requirements.md` §3.1; `docs-v01/Benchmark/10-customer-performance-summary.md` §4), landed for the student/book/course search endpoints and `/me/books`/`/me/courses` behind `shared/paging/CursorPage.java`. `UC-13`/`UC-14`/`UC-15`/`UC-16`'s four diagrams should now be redrawn — `Repo->>DB: SELECT ... LIMIT/OFFSET (+ COUNT for totalElements)` becomes `WHERE <key> > :cursor ORDER BY <key> LIMIT :limit` (no count query at all, per `IP-03`'s coupling note resolving the same way `IP-05` did), and the `200 OK` response shape becomes `{content, nextCursor}`. Not redrawn in this pass — this document's own scope is impact-flagging, not redrawing `docs-v00` diagrams — but the open condition that justified deferring it no longer holds, so this is now a ready-to-do edit rather than a blocked one.
