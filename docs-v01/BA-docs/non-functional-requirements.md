...
### 3.1 Pagination — no use case needs to change

Every use case in `use-cases.md` that paginates anything — UC-13 (student search), UC-14 (book search), UC-15 (course search), UC-16 (own record's books/courses), UC-17 (a student detail's associated list), UC-19 (a course's enrolled-student roster), and UC-20 (look up enrollments) — describes it the same way: "the system returns one page of the matching record(s)... [the actor] may request the next page." Never a page number, never a total count. One of the fix plan's items, `IP-05` (`docs-v01/Benchmark/08-hazard-fix-specs.md`), is still deciding between keeping an absolute "Page N of M" display and moving to Prev/Next-only navigation. **This document set already answers half of that question**: nothing here has ever promised a page count or a jump-to-page capability, so a move to Prev/Next-only navigation requires no change to any UC or user story — it would be dropping an implementation embellishment the business layer never asked for, not withdrawing a stated capability. This is offered as a data point for that still-open decision, not a decision made here.
